...
- **Drizzle strips table qualifiers from columns inside `sql\`\`` in the select list of a single-table query.** A correlated `(SELECT … WHERE client_id = ${clients.id})` renders as `client_id = "id"` and resolves against the inner table → NULL for every row, no error. Use a `LEFT JOIN` on an aggregated subquery (`lib/loyalty/visit-stats.ts`) and assert with `.toSQL()`.
