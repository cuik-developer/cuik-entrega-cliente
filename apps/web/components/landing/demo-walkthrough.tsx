 ...
 *
 *   1. A real hand holds the phone over the counter sign (photo with the
 *      screen cut out, /landing/hand-scan.png). What the camera sees is
 *      rendered BEHIND the photo, inside the measured screen hole (left
  ...
  return (
    <div className="dw flex flex-col lg:flex-row items-center gap-14">
      <style>{`
        ...
        /* Step 1 composition: the hand photo on top, the sign and the camera view behind it */
        .dw-scene { position: absolute; inset: 0; }
        .dw-hand { position: absolute; inset: 0; z-index: 2; pointer-events: none; }
        .dw-hand img { width: 100%; height: 100%; object-fit: contain; filter: drop-shadow(0 24px 30px rgba(15,23,42,0.18)); }
        .dw-scene .dw-sign { position: absolute; left: 0; top: 3%; width: 36%; padding: 0.9rem 0.9rem 0.7rem; z-index: 1; transform: rotate(-4deg); }
        .dw-scene .dw-sign .dw-title { font-size: 0.95rem; }
        /* the screen hole of the photo */
        .dw-screen { position: absolute; left: 38.19%; top: 1.3%; width: 30.57%; height: 67.04%; z-index: 1; border-radius: 17cqw; overflow: hidden; background: radial-gradient(120% 80% at 50% 25%, #f8fafc, #e3e8ee 70%, #cfd6de); container-type: inline-size; }
        .dw-mini { position: absolute; left: 50%; top: 24%; width: 74%; transform: translateX(-50%); border-radius: 8cqw; background: #fff; box-shadow: 0 10cqw 20cqw -8cqw rgba(15,23,42,0.35); padding: 7cqw 7cqw 5cqw; text-align: center; color: #0f172a; }
        .dw-mini .dw-qr { margin-top: 5cqw; padding: 2cqw; border-radius: 3cqw; }
        .dw-frame { position: absolute; left: 50%; top: 41.6%; width: 60%; aspect-ratio: 1; transform: translateX(-50%) scale(1.3); opacity: 0; transition: opacity 240ms var(--dw-out), transform 520ms var(--dw-out); transition-delay: 600ms; pointer-events: none; }
      ...
      <div className="relative flex-shrink-0">
        ...
        <div className="dw-stage relative w-[340px] sm:w-[440px] aspect-[1063/1080]">
          {/* Step 1: the counter sign */}
          <div className={`dw-layer ${step === 0 ? "" : "is-off"}`} aria-hidden={step !== 0}>
            <div className="dw-scene">
              {/* The sign on the counter, behind the hands */}
              ...
              {/* What the camera sees, behind the photo's screen hole */}
              <div className="dw-screen" aria-hidden="true">
              ...
              {/* The hand and phone (screen cut out) */}
              <div className="dw-hand" aria-hidden="true">
                <Image src="/landing/hand-scan.png" alt="" width={1063} height={1080} priority />
              </div>
            </div>
          </div>
