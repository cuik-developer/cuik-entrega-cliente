...
### 8.12 Reportes por correo (semanal / mensual / acumulado)

`apps/web/lib/reports/`:
- `period.ts`: matematica de fechas sobre strings `YYYY-MM-DD` locales (nunca la tz del servidor). `lastClosedWeek(today)` = lunes-domingo anterior; `lastClosedMonth(today)`; `previousWeek/previousMonth/sameMonthLastYear`; `delta()` (porcentaje con base, diferencia absoluta sin base — nunca "sin base"); etiquetas en espanol ("lunes 7 al domingo 13 de setiembre de 2026").
- `compute-report.ts` → `ReportData`. Un solo motor para ambos tipos; el mensual agrega `monthly = { weeks, lastYearVisits, cumulative }`. SQL con `AT TIME ZONE` inline (`tenantTzLiteral`) y limites `>= start::date AND < end::date + 1`. KPIs actual vs anterior; filas diarias zero-filled con hora pico (query aparte agrupada por dia y hora — una subconsulta correlacionada dentro del GROUP BY falla en PG); clientes del periodo con segmento (`computeClientSegment`) y progreso (`cycle_visits` de `visits` vs `promotions.max_visits`, o `points_balance`); campanas `sent` del periodo; en riesgo via `getAtRiskClients`; "accionable" = rewards `pending` (sellos) o clientes con `points_balance >= min(reward_catalog.points_cost)` (puntos); cumpleanos del periodo siguiente via `upcomingBirthdays`. `computeCumulative()` (tambien usado por el export a pedido): totales desde `firstActivityDate` (min de alta del tenant, primer cliente y primera visita), serie mes a mes, top 20 historico, segmentos hoy (`tallySegments`). En programas de puntos los "premios canjeados" son `points_transactions.type = 'redeem'`.
- `compose-email.ts`: puro; `reportSubject`, `reportPreview`, `composeReportEmail(data, urls)` → props de `ReportePeriodico` (secciones como listas de frases; link "Crear campana para clientes en riesgo" → `/panel/clientes?segment=en_riesgo`). Tests con datos fijos.
- `build-excel.ts`: exceljs; hojas segun tipo (Cumpleanos solo si hay filas); encabezados azul Cuik con texto blanco; sin datos de contacto; `addCumulativeSheets()` reutilizado por el export.
- `send-report.ts`: `defaultRecipients()` (contact_email ∪ owner ∪ member owner/admin de la `organization` con `slug` = tenant) y `resolveRecipients()` (la lista `automations.reports.recipients` si no esta vacia, si no la default); `sendReport({ tenantId, kind, to?, period?, force? })` compone, genera el xlsx y llama `sendEmail` con `attachments[{ filename, content: Buffer }]` (el transporte de `@cuik/email` acepta `content` ademas de `path`); guarda `automations.reports.<kind>.lastSentPeriod` salvo en pruebas (`to` explicito). `isReportDue(kind, cfg, dateLocal, hour)` pura; `runDueReports()` recorre tenants active/trial cada hora.
- Config: `reportsAutomationSchema` en `automations-schema.ts` (`weekly { enabled, dayOfWeek 1-7, sendHour, lastSentPeriod? }`, `monthly { enabled, dayOfMonth 1-28, sendHour, lastSentPeriod? }`, `recipients?: string[]` max 10 emails); `PUT /api/[tenant]/automations { reports }` mergea sin pisar `lastSentPeriod`.
