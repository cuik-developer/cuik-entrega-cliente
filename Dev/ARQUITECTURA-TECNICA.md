...
- `build-excel.ts`: exceljs; hojas segun tipo; sin datos de contacto; `addCumulativeSheets()` reutilizado por el export.
- `send-report.ts`: `reportRecipients()` (contact_email ∪ owner ∪ member owner/admin de la `organization` con `slug` = tenant); `sendReport({ tenantId, kind, to?, period?, force? })` compone, genera el xlsx y llama `sendEmail` con `attachments[{ filename, content: Buffer }]` (el transporte de `@cuik/email` acepta `content` ademas de `path`); guarda `automations.reports.<kind>.lastSentPeriod` salvo en pruebas (`to` explicito). `isReportDue(kind, cfg, dateLocal, hour)` pura; `runDueReports()` recorre tenants active/trial cada hora.
- Config: `reportsAutomationSchema` en `automations-schema.ts` (`weekly { enabled, dayOfWeek 1-7, sendHour, lastSentPeriod? }`, `monthly { enabled, dayOfMonth 1-28, sendHour, lastSentPeriod? }`); `PUT /api/[tenant]/automations { reports }` mergea sin pisar `lastSentPeriod`.
...
- UI: `panel/campanas/_components/reports-automation-card.tsx`; `panel/analitica/_components/cumulative-export-button.tsx`.
