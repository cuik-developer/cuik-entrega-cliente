...
**Card "Reportes por correo"** (automatizacion, sep-2026): dos filas, **semanal** y **mensual**, cada una con switch (apagadas por defecto), dia (dia de la semana 1-7 / dia del mes 1-28), hora local del comercio y boton **"Enviarme una prueba"** (manda el ultimo periodo cerrado solo al admin logueado, sin marcarlo como enviado). Abajo, a quien llega: `tenants.contactEmail` + owner + miembros `owner/admin` de la organizacion (deduplicados; nunca cajeros). Config en `tenants.automations.reports.{weekly,monthly}` (`reportsAutomationSchema`), sin cambio de esquema; `lastSentPeriod` guarda el periodo ya enviado para que un cron repetido no duplique. API: `GET/PUT /api/{tenant}/automations` (misma ruta, clave `reports`), `POST /api/{tenant}/reports/send { kind, to? }`.
...
- **Excel adjunto** (`lib/reports/build-excel.ts`, exceljs, **sin telefonos ni emails**): semanal = Resumen · Visitas por dia (con hora pico) · Clientes de la semana · En riesgo · Cumpleanos; mensual agrega Semanas del mes, Campanas, Cumpleanos del mes siguiente y las hojas del acumulado (Resumen historico · Mes a mes · Top 20 historico · Segmentos hoy). La hoja "En riesgo" abre con una nota fija recomendando la campana y cada fila trae "Accion sugerida". Columna "Sellos" (`3 de 6`) o "Puntos" segun el programa.
