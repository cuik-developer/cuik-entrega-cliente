...
- **Filtros globales**: periodo (7 / 30 / 90 dias, este mes, mes pasado, rango libre hasta 366 dias), estado (activos y demos / solo activos / solo demos), programa (sellos / puntos), plan, multiseleccion de comercios y casilla **Incluir demos internas** (apagada por defecto: los tenants marcados en Configuracion → Comercios internos, `global_config.internal_tenant_ids`, quedan fuera de todos los bloques, del selector de comercios, de los pasos por tenant del embudo y del Excel; la barra muestra cuantos se excluyen; `includeInternal=1` en la API). Botones **Exportar con estos filtros** (`format=xlsx`: hojas Resumen, Comercios, Insights) y **Detalle por cliente** (el Excel anterior por tenant, `GET /api/admin/reports/export`).
- **KPIs** (con delta y sparkline): comercios con visitas / comercios del filtro, visitas, clientes nuevos, tasa de retorno (% con 2+ visitas entre los que visitaron), % de clientes nuevos que instalaron el pase, ticket promedio (o canjes si el filtro es sellos) e **ingreso mensual estimado** (`plans.price` **en centimos** / 100 x comercios `active`, en soles; el anterior cuenta los activados hasta el fin del periodo previo: es una estimacion, no hay cobros registrados).
...
**Comercios internos** (sep-2026): tarjeta con la lista de todos los tenants y casillas para marcar los que son demos o pruebas del equipo Cuik (`lib/admin/internal-tenants.ts`, `global_config.internal_tenant_ids`). Metricas los excluye por defecto (ver §3.6). Acciones `getInternalTenantsConfig` / `saveInternalTenants`.
...
- Tabs:
  - **Actividad** (por defecto): timeline unificado, mas reciente primero, con filtros Todo / Visitas / Premios / Notas / Campanas. Eventos: registro, visita (sello N, ciclo, sucursal, monto, `+N pts` solo en programas de puntos, "registro manual"/"bonus", cajero), premio ganado (con vencimiento), premio canjeado, premio vencido sin canjear, nota (con autor), bloqueo/desbloqueo (evento propio, detectado por el contenido de la nota), campana enviada/entregada/fallida (con el mensaje). `GET /api/{tenant}/clients/{id}/timeline?limit=N` (default 100, max 500; hasta 200 filas por fuente) — `lib/crm/client-timeline.ts`. Se recarga solo tras bloquear/desbloquear. En programas de puntos (sep-2026) la visita se titula "Visita · +N pts" / "Bono de registro · +N pts", los canjes salen de `points_transactions` ("Canjeo N pts: <premio>", con el cajero de `metadata.cashierId`) y se omiten las filas de `loyalty.rewards`, que ahi son un subproducto del canje.
  - **Puntos** (solo programas de puntos, sep-2026): estado de cuenta. Tarjetas Saldo actual / Ganados en total / Canjeados en total y tabla de movimientos (mas reciente primero) con fecha, tipo (Compra, Bono de registro, Cumpleanos, Promocion, Canje, Vencimiento, Ajuste), detalle (premio canjeado, monto de la compra, "base + extra" en cumpleanos/promocion), puntos con signo, **saldo despues del movimiento** y quien atendio (cajero de la visita o del canje). `GET /api/{tenant}/clients/{id}/points?limit=N` (default 200, max 1000) — `lib/crm/client-points.ts`; el saldo por fila se recalcula hacia atras desde `clients.points_balance`, asi que es consistente aunque la fila sea anterior a `metadata.balanceAfter`.
...
**Card "Reportes por correo"** (al pie de Analitica, sep-2026): dos filas, **semanal** y **mensual**, cada una con switch (apagadas por defecto), dia (dia de la semana 1-7 / dia del mes 1-28), hora local del comercio y boton **"Enviarme una prueba"** (manda el ultimo periodo cerrado solo al admin logueado, sin marcarlo como enviado). Bloque **"A quien le llegan"**: lista propia de hasta 10 correos (chips con quitar, "Sugeridos" con un clic = `tenants.contactEmail` + owner + miembros `owner/admin` de la organizacion, y un input para escribir cualquier otro). Lista vacia = se usa la sugerida. Config en `tenants.automations.reports.{weekly,monthly,recipients}` (`reportsAutomationSchema`), sin cambio de esquema; `lastSentPeriod` guarda el periodo ya enviado para que un cron repetido no duplique. API: `GET/PUT /api/{tenant}/automations` (misma ruta, clave `reports`), `POST /api/{tenant}/reports/send { kind, to? }`.
- **Contenido del correo** (`ReportePeriodico`, `packages/email`): asunto con el dato fuerte ("Mascota Veloz: 42 visitas esta semana (+14 %)" / "Mascota Veloz en setiembre: 168 visitas (+9 %)"); tres cifras con comparacion contra el periodo anterior (visitas, clientes nuevos, premios canjeados — en puntos, "Canjes de puntos"); "Lo que paso" (dia mas fuerte y mas flojo, clientes distintos y repetidores, campanas push enviadas; en el mensual ademas mejor semana y mismo mes del ano pasado si hay data); "Tus clientes mas fieles"; "Para actuar" (clientes en riesgo con la **recomendacion de enviarles una campana** y link a Clientes con el segmento En riesgo, premios pendientes de canje o clientes con saldo para canjear en puntos, cumpleanos del periodo que empieza y si el push automatico esta activo); en el mensual, "Desde que empezaste" (totales historicos, promedio de visitas por cliente, mejor mes, top 5 historico). Boton al panel y pie con donde se configura.
- **Excel adjunto** (`lib/reports/build-excel.ts`, exceljs, encabezados en azul Cuik `#0e70db`, **sin telefonos ni emails**): semanal = Resumen · Visitas por dia (con hora pico) · Clientes de la semana · En riesgo · Cumpleanos (solo si hay alguno); mensual agrega Semanas del mes, Campanas, Cumpleanos del mes siguiente y las hojas del acumulado (Resumen historico · Mes a mes · Top 20 historico · Segmentos hoy). La hoja "En riesgo" abre con una nota fija recomendando la campana y cada fila trae "Accion sugerida". Columna "Sellos" (`3 de 6`) o "Puntos" segun el programa.
...
- **Nuevo / Editar premio** (dialogo): foto (PNG/JPG hasta 5 MB; la foto se muestra siempre en un marco 16:9 arrastrable con zoom, `_components/photo-adjuster.tsx`, tanto al elegirla como al reabrir un premio guardado; al guardar el premio, si es nueva o se movio, se recorta en canvas a 1200x675 y se sube el PNG via `POST /api/{tenant}/assets/upload`, guardando su URL relativa `/api/assets/...`), nombre, descripcion, costo en puntos (entero > 0), categoria, orden y visible.
...
- El cumpleanos del cliente lo carga el cliente en el registro publico (si el super-admin activo el switch **"Cumpleanos"** en Tenants → Configuracion de registro, con opcion "Obligatorio"; se guarda en `registrationConfig.birthday = { enabled, required, label? }` y `buildRegistrationSchema` lo exige/valida como `YYYY-MM-DD`. `label` (sep-2026, opcional, max 60) personaliza la pregunta del formulario publico, p. ej. "Cumpleanos de tu engreid@" en una petshop; vacio = "Fecha de cumpleanos". El resumen de Tenants → Registro muestra el bloque Cumpleanos (pregunta, obligatorio) y la variable `{{client.birthday}}`. Un tenant sin cumpleanos cargados (p. ej. base historica de 400+ clientes) usa Campanas con normalidad: si el registro no pregunta el cumpleanos, nadie lo tiene cargado y el saludo esta apagado, la tarjeta del saludo **no se muestra** (evita confundir al comercio); si el registro si lo pregunta pero aun no hay fechas, muestra un aviso ambar, `GET /api/{tenant}/automations` devuelve `birthday.asked` (si el registro pregunta) y cada bloque de esa ruta degrada por separado (nunca 500 por un widget). No es un campo estrategico: `birthday` es clave reservada) o el admin desde la ficha, pestana Informacion, campo "Cumpleanos" con editor inline (`PATCH /api/{tenant}/clients/{id}` `{ birthday: "YYYY-MM-DD" | null }`).
...
| **Reporte semanal / mensual** | Cron `reports` cuando toca, o "Enviarme una prueba" desde Campanas | `ReportePeriodico` + xlsx adjunto | contactEmail + owner + admins del tenant (la prueba, solo al admin logueado) |
